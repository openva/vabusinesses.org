...
class Page
{
	...
	public function load_template($template_file)
	{
		...
		/*
		 * Save the contents of the template file to a variable. First check Memcached and see if
		 * it's stored there, if we've enabled the use of Memcached.
		 */
		$storage_name = 'template-' . $this->theme_name . '-' . $this->page;
		if ( MEMCACHED === TRUE)
		{
		
			$html = $mc->get($storage-name);
			if ($html != FALSE)
			{
			...
			else
			{
				...
				/*
				 * Cache the template for one day.
				 */
				$mc->set($storage_name, serialize($html), 86400);
	...
	public function render($content)
	{
		...
		/*
		 * Make a copy of the template here, so we can re-render as often as we like with new
		 * content.
		 */
		$template = $this->html;
